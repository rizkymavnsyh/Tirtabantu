...
import { userList, wilayahList, User } from "../../data/mockData";
...
export function ManajemenPengguna() {
  ...
  const roleIcon = (role: string) => {
    ...
    if (role === "petugas") return <Wrench className="w-4 h-4 text-emerald-600" />;
    return <UsersIcon className="w-4 h-4 text-amber-600" />;
  };
    ...
    <div>
      ...
      <div className="bg-white rounded-xl border border-sky-100 shadow-sm overflow-x-auto">
        <table className="w-full" style={{ fontSize: "0.85rem" }}>
          ...
          <tbody>
            {data.map((u) => {
              const wil = wilayahList.find((w) => w.id === u.wilayah_id);
              return (
                <tr key={u.id} className="border-b border-sky-50 hover:bg-sky-50/30">
                  ...
                  <td className="p-4 text-slate-500">{wil?.nama || "-"}</td>
