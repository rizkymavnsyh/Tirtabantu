...
import { Plus, Pencil, Trash2, Megaphone, AlertTriangle, Clock, Info, Siren } from "lucide-react";

const kategoriOptions = [
  { value: "darurat", label: "Darurat", color: "bg-red-100 text-red-700" },
  { value: "gangguan", label: "Gangguan", color: "bg-amber-100 text-amber-700" },
  { value: "jadwal", label: "Jadwal", color: "bg-blue-100 text-blue-700" },
  { value: "info", label: "Informasi", color: "bg-emerald-100 text-emerald-700" },
];

export function PengumumanAdmin() {
  const [data, setData] = useState<Pengumuman[]>([...pengumumanList]);
  const [showForm, setShowForm] = useState(false);
  const [form, setForm] = useState({ judul: "", isi: "", penting: false, kategori: "info" as Pengumuman["kategori"] });

  const handleSave = () => {
    if (!form.judul || !form.isi) return;
    setData([{ id: Date.now(), tgl_posting: "2026-03-16", ...form }, ...data]);
    setForm({ judul: "", isi: "", penting: false, kategori: "info" });
    setShowForm(false);
  };

  const getKategoriStyle = (kat: string) => kategoriOptions.find(k => k.value === kat) || kategoriOptions[3];
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-500" style={{ fontSize: "0.85rem" }}>Kelola info gangguan distribusi air dan pengumuman publik</p>
        </div>
      ...
      {showForm && (
        <div className="bg-white rounded-xl p-5 border border-sky-100 shadow-sm mb-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="text-sky-800 mb-1 block" style={{ fontSize: "0.85rem" }}>Kategori</label>
              <div className="flex gap-2 flex-wrap">
                {kategoriOptions.map(k => (
                  <button
                    key={k.value}
                    type="button"
                    onClick={() => setForm({ ...form, kategori: k.value as Pengumuman["kategori"] })}
                    className={`px-3 py-1.5 rounded-lg border-2 transition-all ${
                      form.kategori === k.value ? `${k.color} border-current` : "border-sky-100 text-slate-500"
                    }`}
            ...
            <div>
              ...
              <textarea value={form.isi} onChange={(e) => setForm({ ...form, isi: e.target.value })} className="w-full px-3 py-2 border border-sky-200 rounded-lg bg-sky-50/50 focus:outline-none focus:ring-2 focus:ring-sky-300 h-28 resize-none" style={{ fontSize: "0.85rem" }} />
            </div>
      ...
      <div className="space-y-4">
        {data.map((p) => {
          const katStyle = getKategoriStyle(p.kategori);
          return (
            <div key={p.id} className={`bg-white rounded-xl p-5 border shadow-sm ${p.penting ? "border-amber-200" : "border-sky-100"}`}>
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3 mb-2 flex-wrap">
                  <Megaphone className="w-5 h-5 text-sky-500" />
                  <span className={`${katStyle.color} px-2.5 py-0.5 rounded-full`} style={{ fontSize: "0.72rem", fontWeight: 700 }}>
                    {katStyle.label.toUpperCase()}
                  </span>
                  {p.penting && (
                    <span className="bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full flex items-center gap-1" style={{ fontSize: "0.7rem", fontWeight: 600 }}>
                      <AlertTriangle className="w-3 h-3" /> PENTING
                    </span>
                  )}
                  <span className="text-slate-400" style={{ fontSize: "0.78rem" }}>{p.tgl_posting}</span>
                </div>
